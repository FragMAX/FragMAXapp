...
import subprocess
import celery
from fragview.versions import HZB_ADXV
...
def _make_diffraction_image_from_cbf(cbf_file, dest_pic_file):
    if path.exists(dest_pic_file):
        # diffraction picture already generated
        return

    cmd = [f"{HZB_ADXV}", "-sa", cbf_file,  dest_pic_file]
    subprocess.run(cmd)


@celery.task
def get_diffraction(diff_source_file, dest_pic_file):
    """
    ...
    with dist_lock.acquire(lock_id):
        _make_diffraction_image_from_cbf(diff_source_file, dest_pic_file)
