...
class Project:
    ...
    def _load_project_fields(self):
        proj_model = self.db.Project.select().first()
        self._protein = proj_model.protein
        self._proposal = proj_model.proposal
    ...
    def get_data_collection_dates(self):
        """
        get datetime(s) for when earliest and lastes dataset was recorded
        """
    ...
    def project_dir(self) -> Path:
        return Path(conf.PROJECTS_ROOT_DIR, f"proj{self.id}")
...
def create_project(project_id: str, proposal, protein) -> Project:
    return Project(create_project_db(project_id, proposal, protein), project_id)


def get_project(project_id) -> Project:
    return Project(get_project_db(project_id), project_id)
