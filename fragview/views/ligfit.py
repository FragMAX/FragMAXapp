...
def datasets(request):
    ...
    empty, rhofitSW, ligfitSW, fitprocess, scanchirals, customrhofit, customligfit, filters = userInput.split(";;")
...
def auto_ligand_fit(proj, useLigFit, useRhoFit, filters):
    ...
    for num, pdb in enumerate(pdbList):
        rhofit_cmd = ""
        ligfit_cmd = ""
        fragID = pdb.split("/")[7].split("-")[-1].split("_")[0]
        ...
        if "ligfit" in fitmethod:
            proc, ref = ligfit_outdir.split("/")[-4:-2]
            ligandfit_log = f"{proj.data_path()}/fragmax/logs/LigandFit_{proc}_{ref}_{sample}_out.txt"
            ...
            ligfit_cmd += f"phenix.ligandfit data={mtz_input} model={pdb} ligand={ligPDB} fill=True clean_up=True | " \
                          f"dd status=none of={ligandfit_log}\n"
