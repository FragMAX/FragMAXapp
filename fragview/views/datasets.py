...
def import_edna_fastdp(proj):
    ...
    h5s=list(itertools.chain(*[glob(f"/data/visitors/biomax/{proj.proposal}/{shift}/raw/{proj.protein}/{proj.protein}*/{proj.protein}*master.h5") for shift in proj.shifts()]))
    for h5 in h5s:
        dataset,run=(h5.split("/")[-1][:-10].split("_"))
        shift_collection =  h5.split("/")[5]
        edna_path_src    =  f"/data/visitors/biomax/{proj.proposal}/{shift_collection}/process/{proj.protein}/{dataset}/xds_{dataset}_{run}_1/EDNA_proc/results/"
        edna_path_dst    =  f"{proj.data_path()}/fragmax/process/{proj.protein}/{dataset}/{dataset}_{run}/edna/"
        fastdp_path_src  =  f"/data/visitors/biomax/{proj.proposal}/{shift_collection}/process/{proj.protein}/{dataset}/xds_{dataset}_{run}_1/fastdp/results/"
