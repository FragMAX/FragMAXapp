import re
from os import path
from django.forms import Form, CharField, BooleanField, IntegerField, FileField, ValidationError
...
class ProjectForm(Form):
    crystal_csv_file = FileField()
    protein = CharField()
    proposal = CharField()
    autoproc = BooleanField(required=False)
    encrypted = BooleanField(required=False)

    def clean_crystal_csv_file(self):
        csv_file = self.cleaned_data["crystal_csv_file"]
        try:
    ...
    def get_error_message(self):
        csv_errors = self.errors.get("crystal_csv_file")
        if csv_errors:
    ...
    def get_values(self):
        ...
        return cdata["protein"], cdata["proposal"], cdata["crystal_csv_file"],\
               cdata["autoproc"], cdata["encrypted"]
