...
class TestNoProjectsRedirect(unittest.TestCase):
    """
    test no_projects_redirect() middleware
    """
    def setUp(self):
        self.get_resp_mock = mock.Mock()
        self.middleware_callable = \
            middleware.no_projects_redirect(self.get_resp_mock)
    ...
    def _request_mock(self, path="/", with_project=False):
        req = mock.Mock()
        req.path = path
